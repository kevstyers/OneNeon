...
sort2018 <- sort %>%
  filter(bet_sorting.setDate > "2018-01-01" & bet_sorting.setDate < "2019-01-01") %>%
...
sort2018f <- within(sort2018f, 
                    bet_sorting.scientificName <- factor(bet_sorting.scientificName, 
                                                  levels=names(sort2018f(table(bet_sorting.scientificName), 
                                                                     decreasing=FALSE))))

plot<- ggplot(sort2018f, aes(x= bet_sorting.setDate,fill = bet_sorting.identifiedBy))+
  geom_density(alpha = .2) +
  labs(title = "Total Beetles Counts by Plot ID for D10.13 in 2018",
       x = "Domain",
       y = "Count") +
  theme(axis.text.x = element_text(angle = 270)) 
#facet_wrap(~year)
